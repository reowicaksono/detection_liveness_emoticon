...
def visualize(image, det_res, fer_res, box_color=(0, 255, 0), text_color=(0, 0, 255)):
    ...
    for ind, (det, fer_type) in enumerate(zip(det_res, fer_res)):
        ...
        try :
            def face_img(path):
                face_image = cv.imread(path)
                face_image = cv.resize(face_image, (bbox[2], bbox[3]))
                output[bbox[1]:bbox[1]+bbox[3], bbox[0]:bbox[0]+bbox[2]] = face_image

            if fer_type == 'neutral' :
               print('ini neutral')
               # remove_background('../facial_expression_recognition/emoticon/neutral.png', '../facial_expression_recognition/emoticon/neutral2.png')
               face_image_path = "../facial_expression_recognition/emoticon/neutral.png"
               face_img(face_image_path)

            elif fer_type == 'happy' :
                print('ini happy')
                face_image_path = "../facial_expression_recognition/emoticon/happy.png"
                face_img(face_image_path)
            
            elif fer_type == 'disgust' :
                print('ini disgust')
                face_image_path = "../facial_expression_recognition/emoticon/disgusted.png"
                face_img(face_image_path)

            elif fer_type == 'sad' :
                print('ini sad')
                face_image_path = "../facial_expression_recognition/emoticon/sad.png"
                face_img(face_image_path)

            elif fer_type == 'surprised' :
                print('ini suprised')
                face_image_path = "../facial_expression_recognition/emoticon/surprised.png"
                face_img(face_image_path)
        ...
        # Load the face image with transparency
        # face_image_path = "../facial_expression_recognition/emoticon/smile.png"
        # # face_image = cv.imread(face_image_path, cv.IMREAD_UNCHANGED)

        # # # Resize the face image
        # # face_image = cv.resize(face_image, (bbox[2], bbox[3]))

        #  # Load the face image
        # face_image = cv.imread(face_image_path)
        # face_image = cv.resize(face_image, (bbox[2], bbox[3]))

        # # Paste the face image into the output image
        # output[bbox[1]:bbox[1]+bbox[3], bbox[0]:bbox[0]+bbox[2]] = face_image
